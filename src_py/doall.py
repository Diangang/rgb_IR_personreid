...
def train(model, train_data, criterion, optimizer, epoch):
    ...
    torch.save(model.cpu().state_dict(), os.path.join(opt['save'], 'deep_zero_model#' + str(epoch) + '.pth'))
...
if __name__ == '__main__':
    ...
    test_dataset = Dataset(opt['dataroot'], test_ids, 'test')
    
    train_dataloader = data.DataLoader(train_dataset, batch_size=opt['batch_size'], shuffle=True)

    model = resnet6(num_classes=len(train_ids), pretrained_model=pretrained_model)
