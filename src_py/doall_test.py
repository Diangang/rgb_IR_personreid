...
pretrained_model = "../scratch/sysu_mm01/deepzeropadding-08Apr2018-164359_deep-zero-padding/deep_zero_model#190.pth"
# init the settings
...
def test(model, test_dataset, test_ids):
    ...
    for cam_name, id_contents in data_instances.items():
        matfile_path = os.path.join(testresults_dir, matfile_prefix + '_' + cam_name + '.mat')
        cam_features = prepare_empty_matfile_config(max_test_id)
        
        for id_, img_contents in id_contents.items():
            all_current_id_features = np.empty(shape=[0, 512])
            for img_config in img_contents:
                ...
                all_current_id_features = np.append(all_current_id_features, current_feature, axis=0)
...
if __name__ == '__main__':
    ...
    model = resnet6(num_classes=len(train_ids))
