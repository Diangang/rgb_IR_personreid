...
-- to return a random file path from name -- path hash
function select_random_sample(data_hash)
    all_keys = table.getAllKeys(data_hash)
    random_key = all_keys[getRandomNumber(1, #all_keys)]
    return data_hash[random_key]
end

function read_and_augment_image(filepath)
    --TEMPORARY WORKAROUND:
    -- if the file path is a tensor, dont read the image
    if nil == torch.typename(filepath) then
        img = image.load(filepath)
        img = image.scale(img, 60, 160)
    else
        img = filepath
    end
    ...
    if torch.uniform() < 0.5 then
        ...
        img = image.translate(img, randomWidth, randomHeight)
    end
...
function read_and_augment_images(filepaths)
    return {read_and_augment_image(filepaths[1]),
            read_and_augment_image(filepaths[2])}
end

function read_image_and_resize(filepath, isIR)
    img = image.load(filepath)
    -- if IR image, take only one channel
    if isIR then img = img[{{1},{},{}}]; end
    img = image.scale(img, 60, 160)
    return img
end
    ...
    for index = 1, totalBatches do
        ...
        for dataIndex = 1, count do
           ...
           cutorch.withDevice(dataIndex, function() 
                                            image_data = read_and_augment_images(data[currentDataIndex])
                                            clonedData = cudaTheData(image_data)
                                        end)
