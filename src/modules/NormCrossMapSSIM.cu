...
bool NormCrossMapSSIM_IsMallocSet = false;
...
static void setMallocLimit() {
	...
	if(!NormCrossMapSSIM_IsMallocSet)
	{
		...
		NormCrossMapSSIM_IsMallocSet = true;
	}
}


__device__
void NCMSSIM_PrintArray(float* array, int volume, int rows, int columns)
{
...
__device__
void NCMSSIM_setArrayContentsToZero(float* array, int numRows, int numColumns)
{
...
__device__ void NCMSSIM_getPatchWithMidPoint(float* input, int layerNumber, int rowNumber, int columnNumber, int numLayers, int mapHeight, int mapWidth, int patchwidth, float* focusPatch)
{
...
__device__
void NCMSSIM_getMeanAndStd(float* meanMaps, float* stdMaps, int mapHeight, int mapWidth, int patchwidth, int layerNumber, int rowNumber, int columnNumber, float* mean, float* std)
{
...
__device__ float NCMSSIM_meanCorrelation(float* input, int layerNumber, int rowNumber, int columnNumber, int numLayers, int mapHeight, int mapWidth, int patchwidth, int verticalWidth, float* focusPatch, float correlatorMean, float correlatorStd, float* meanMaps, float* stdMaps)
{
    int halfPatchWidth = patchwidth / 2;
    ...
    NCMSSIM_getMeanAndStd(meanMaps, stdMaps, mapHeight, mapWidth, patchwidth, layerNumber, rowNumber, columnNumber, &correlateeMean, &correlateeStd);
    ...
    for(int rowIndex = -halfPatchWidth; rowIndex <= halfPatchWidth; rowIndex++)
    {
        ...
        for(int columnIndex = -halfPatchWidth; columnIndex <= halfPatchWidth; columnIndex++)
        {
			...
			correlatedValue += ((correlatorVal - correlatorMean) * (correlateeVal - correlateeMean));
        }
    }
    
    return correlatedValue / N;
}

__device__ float NCMSSIM_ssimPatch(float* input, int layerNumber, int rowNumber, int columnNumber, 
                                   int numLayers, int mapHeight, int mapWidth, int patchwidth, int verticalWidth, 
                                   float* focusPatch, float correlatorMean, float correlatorStd, float correlatedValue,
                                   float* meanMaps, float* stdMaps)
{
    float correlateeMean = 0, correlateeStd = 0;
    ...
    //calculate parts of SSIM
    float A = correlatorMean * correlateeMean;
    float B = correlatedValue;
    float C = (correlatorMean * correlatorMean) + (correlateeMean * correlateeMean);
    float D = (correlatorStd * correlatorStd) + (correlateeStd * correlateeStd);
    
    return (4 * A * B) / (C * D);
}

__device__ 
void NCMSSIM_ConstrainedSSIM(float* input, float* focusPatch, int layerNumber, int rowNumber, int columnNumber, 
                             int numLayers, int mapHeight, int mapWidth, float* output, float* meanCorrelation,
                             int patchwidth, int verticalWidth, float* meanMaps, float* stdMaps)
{
    ...
    NCMSSIM_getMeanAndStd(meanMaps, stdMaps, mapHeight, mapWidth, patchwidth, layerNumber - halflayerspoint, rowNumber, columnNumber, &correlatorMean, &correlatorStd);
    ...
    { 
        ...
        {
            float correlatedValue = NCMSSIM_meanCorrelation(input, layerNumber, currentRowIndex, columnIndex, 
                                                            numLayers, mapHeight, mapWidth, patchwidth, verticalWidth, 
                                                            focusPatch, correlatorMean, correlatorStd, 
                                                            meanMaps, stdMaps);
                                                            
            ELEMENT(meanCorrelation, (layerNumber - halflayerspoint) * verticalWidth * mapWidth + rowIndex * mapWidth + columnIndex, 
                            rowNumber, columnNumber, mapHeight, mapWidth) = correlatedValue;
            
            float ssimValue = NCMSSIM_ssimPatch(input, layerNumber, currentRowIndex, columnIndex, 
                                                            numLayers, mapHeight, mapWidth, patchwidth, verticalWidth, 
                                                            focusPatch, correlatorMean, correlatorStd, 
                                                            correlatedValue, meanMaps, stdMaps);

            //store the correlatedvalue in output
            ELEMENT(output, (layerNumber - halflayerspoint) * verticalWidth * mapWidth + rowIndex * mapWidth + columnIndex, rowNumber, columnNumber, mapHeight, mapWidth) = ssimValue;
...
__global__
void NCMSSIM_calcNormCrossMapSSIM(float* input, int numLayers, int mapHeight, int mapWidth, 
                                float* output, float* meanCorrelation, int patchwidth, int verticalWidth, float* meanMaps, float* stdMaps)
{
   ...
   // if(!(layerNumber == 0 && rowNumber == 0 && columnNumber == 0)) return;
    ...
    float focusPatch[25];
    NCMSSIM_setArrayContentsToZero(focusPatch, patchwidth, patchwidth);
    
    NCMSSIM_getPatchWithMidPoint(input, layerNumber, inputRowNumber, inputColumnNumber, numLayers, mapHeight, mapWidth, patchwidth, focusPatch);
    ...
    NCMSSIM_ConstrainedSSIM(input, focusPatch, layerNumber + halflayerspoint, inputRowNumber, inputColumnNumber, 
                            numLayers, mapHeight, mapWidth, output, meanCorrelation, patchwidth, verticalWidth, meanMaps, stdMaps);

}

__global__
void NCMSSIM_calcMeanAndStdMaps(float* input, int numLayers, int mapHeight, int mapWidth, int patchwidth, int verticalWidth, float* meanMaps, float* stdMaps) 
{
    //calculate the layer index, row index, column index    
    int blockNumber = blockIdx.x;
    int elementIndex = threadIdx.x;
    ...
    NCMSSIM_getPatchWithMidPoint(input, layerNumber, inputRowNumber - halfVericalWidth, inputColumnNumber, numLayers, mapHeight, mapWidth, patchwidth, focusPatch);
    ...
    ELEMENT(meanMaps, layerNumber, inputRowNumber, inputColumnNumber, mapHeight + 4, mapWidth) = (sum / (patchwidth * patchwidth)) + 1e-2;
    ...
    float std = sqrt(variance / (patchwidth * patchwidth)) + 1e-2;
	ELEMENT(stdMaps, layerNumber, inputRowNumber, inputColumnNumber, mapHeight + 4, mapWidth) = std;
}

void NCMSSIM_updateOutput(THCState *state, THCudaTensor *input, THCudaTensor *output, THCudaTensor *meanCorrelation, 
                            int patchwidth, int verticalWidth, THCudaTensor *meanMaps, THCudaTensor *stdMaps) {

	THCUNN_assertSameGPU(state, 3, input, output, meanCorrelation);
	...
	NCMSSIM_calcMeanAndStdMaps<<<totalBlocks, totalThreads>>>(inputContents, numLayers, mapHeight, mapWidth, patchwidth, verticalWidth, meanMapsPtr, stdMapsPtr);
		
	THCudaTensor_resize3d(state, output, outputNumLayers, outputMapHeight, outputMapWidth);
    THCudaTensor_fill(state, output, 0);
	THCudaTensor_resize3d(state, meanCorrelation, outputNumLayers, outputMapHeight, outputMapWidth);
    THCudaTensor_fill(state, meanCorrelation, 0);

	//get c-pointer address
	float* outputPtr = THCudaTensor_data(state, output);
    float* meanCorrelationPtr = THCudaTensor_data(state, meanCorrelation);

	totalBlocks = (numLayers / 2);
	totalThreads = mapHeight * mapWidth;

    NCMSSIM_calcNormCrossMapSSIM<<<totalBlocks, totalThreads>>>(inputContents, numLayers, mapHeight, mapWidth, 
                                                                outputPtr, meanCorrelationPtr, patchwidth, verticalWidth, meanMapsPtr, stdMapsPtr);

}

__device__
float NCMSSIM_getSSIMValue(float* output, int layerIndex, int mapHeight, int mapWidth, int verticalWidth, int relativeCorrelateeRowIndex, int relativeCorrelateeColumnIndex, int currentMapRowIndex, int currentMapColumnIndex)
{
	float ssimValue = 0;
	ssimValue = ELEMENT(output, (layerIndex * verticalWidth * mapWidth) + (relativeCorrelateeRowIndex * mapWidth) + relativeCorrelateeColumnIndex, 
...
__device__
float NCMSSIM_getMeanCorrelationValue(float* meanCorrelation, int layerIndex, int mapHeight, int mapWidth, int verticalWidth, int relativeCorrelateeRowIndex, int relativeCorrelateeColumnIndex, int currentMapRowIndex, int currentMapColumnIndex)
{
	float correlationValue = 0;
	correlationValue = ELEMENT(meanCorrelation, (layerIndex * verticalWidth * mapWidth) + (relativeCorrelateeRowIndex * mapWidth) + relativeCorrelateeColumnIndex, 
                                currentMapRowIndex, currentMapColumnIndex, mapHeight, mapWidth);
...
__device__
void NCMSSIM_retrieveOutputGradient(float* gradOutput, float* currentOutputGradient, int layerNumber, int currentMapRowIndex, int currentMapColumnIndex, int mapHeight, int mapWidth, int patchwidth, int verticalWidth)
{
...
__device__
float NCMSSIM_retrieveAndMultiplyOutputGradWithCorrelator(float* gradOutput, float* currentCorrelator, int layerNumber, int currentMapRowIndex, int currentMapColumnIndex, int mapHeight, int mapWidth, int patchwidth, int verticalWidth)
{
	int startLayerNumber = layerNumber * verticalWidth * mapWidth;
...
__device__
float NCMSSIM_gradSSIMbasedOnX(float x_i, float mu_x, float sigma_x, 
                               float y_i, float mu_y, float sigma_y, float meanCorrelationValue, float ssimValue, int N)
{
    //calculate parts of SSIM
    float A = mu_x * mu_y;
    float B = meanCorrelationValue;
    float C = (mu_x * mu_x) + (mu_y * mu_y);
    float D = (sigma_x * sigma_x) + (sigma_y * sigma_y);
    
    float x_i_minus_mu_x = x_i - mu_x;
    float y_i_minus_mu_y = y_i - mu_y;
    
    return (ssimValue/N) * ((y_i_minus_mu_y/ B) + 
                          (mu_y / A) - 
                          (2 * mu_x / C) -
                          (2 * x_i_minus_mu_x / D));
}
...
__device__
float NCMSSIM_gradSSIMbasedOnY(float x_i, float mu_x, float sigma_x, 
                               float y_i, float mu_y, float sigma_y, float meanCorrelationValue, float ssimValue, int N)
{
    //calculate parts of SSIM
    float A = mu_x * mu_y;
    ...
    return (ssimValue/N) * ((x_i_minus_mu_x/ B) + 
                          (mu_x / A) - 
                          (2 * mu_y / C) -
                          (2 * y_i_minus_mu_y / D));
}
...
__device__
void NCMSSIM_gradientWrtInput(float* input, float* output, float* meanCorrelation, float* currentGradInput, int currentMapRowIndex, int currentMapColumnIndex, 
                            int layerNumber, int wrtRowNumber, int wrtColumnNumber, int numLayers, int mapHeight, int mapWidth, 
                            int patchwidth, int verticalWidth, float* meanMaps, float* stdMaps)
{
    int halfPatchWidth = patchwidth / 2;
    int halfVerticalWidth = verticalWidth / 2;
    ...
    NCMSSIM_getMeanAndStd(meanMaps, stdMaps, mapHeight, mapWidth, patchwidth, layerNumber - halflayerspoint, currentMapRowIndex, currentMapColumnIndex, &x_bar, &sigma_x);
    ...
    for(int rowIndex = 0; rowIndex < verticalWidth; rowIndex++)
    {
        ...
        for(int columnIndex = 0; columnIndex < mapWidth; columnIndex++)
        {
            ...
            NCMSSIM_getMeanAndStd(meanMaps, stdMaps, mapHeight, mapWidth, patchwidth, layerNumber, currentRowIndex + corrTorTeeRowDiff, columnIndex, &y_bar, &sigma_y);
            
            float ssimValue = NCMSSIM_getSSIMValue(output, layerNumber - halflayerspoint, mapHeight, mapWidth, verticalWidth, 
												currentRowIndex + corrTorTeeRowDiff - currentMapRowIndex + halfPatchWidth, columnIndex, 
												currentMapRowIndex, currentMapColumnIndex);
            float meanCorrelationValue = NCMSSIM_getMeanCorrelationValue(meanCorrelation, layerNumber - halflayerspoint, mapHeight, mapWidth, verticalWidth, 
                                    currentRowIndex + corrTorTeeRowDiff - currentMapRowIndex + halfPatchWidth, columnIndex, 
                                    currentMapRowIndex, currentMapColumnIndex);
            ...
            ELEMENT(currentGradInput, 0, rowIndex, columnIndex, 
                                        verticalWidth, mapWidth) = NCMSSIM_gradSSIMbasedOnX(x_i, x_bar, sigma_x, y_i, y_bar, 
                                                                                            sigma_y, meanCorrelationValue, ssimValue, N);
        }
    }
...
__device__
void NCMSSIM_getGradientForTopLayer(float* input, float* output, float* meanCorrelation,
                                    int layerNumber, int rowNumber, int columnNumber, int numLayers, int mapHeight, int mapWidth, 
                                    float* gradOutput, float* gradInput, int patchwidth, int verticalWidth, 
                                    float* meanMaps, float* stdMaps)
{
    int halfPatchWidth = patchwidth / 2;
    int halflayerspoint = numLayers / 2;
    ...
    for(int columnIndex = -halfPatchWidth; columnIndex <= halfPatchWidth; columnIndex++)
    {
    	...
    	for(int rowIndex = -halfPatchWidth; rowIndex <= halfPatchWidth; rowIndex++)
        {
           ...
           //if the row and column index is valid, get the corresponding gradient
            ...
            {
                NCMSSIM_setArrayContentsToZero(currentCorrelator, verticalWidth, mapWidth);
                NCMSSIM_gradientWrtInput(input, output, meanCorrelation, currentCorrelator, currentMapRowIndex, currentMapColumnIndex, 
            	...
            	inputGradient += NCMSSIM_retrieveAndMultiplyOutputGradWithCorrelator(gradOutput, currentCorrelator, 
                                                                                layerNumber, currentMapRowIndex, currentMapColumnIndex,
                                                                                mapHeight, mapWidth, patchwidth, verticalWidth);
...
__device__
float NCMSSIM_retrieveGradientWrtCorrelateeInput(float* input, float* output, float* meanCorrelation, float* currentGradientForBottom, int layerNumber, int currentRowIndex, 
												...
												int mapWidth, int patchwidth, int verticalWidth, float* meanMaps, float* stdMaps)
{
    ...
    NCMSSIM_getPatchWithMidPoint(input, layerNumber, currentRowIndex, currentColumnIndex, numLayers, mapHeight, mapWidth, patchwidth, focusPatch);
    float x_bar = 0, sigma_x = 0;
    NCMSSIM_getMeanAndStd(meanMaps, stdMaps, mapHeight, mapWidth, patchwidth, layerNumber, currentRowIndex, currentColumnIndex, &x_bar, &sigma_x);
    ...
    for(int reverseRowIndex = startIndex; reverseRowIndex <= endIndex; reverseRowIndex++)
    {
        ...
        for(int reverseColumnIndex = 0; reverseColumnIndex < patchwidth; reverseColumnIndex++)
        {
            ...
            if(inputGradientColumnNumber >= 0 && inputGradientColumnNumber < mapWidth) {
                ...
                float ssimValue = NCMSSIM_getSSIMValue(output, layerNumber, mapHeight, mapWidth, verticalWidth, 
                ...
                float meanCorrelationValue = NCMSSIM_getMeanCorrelationValue(meanCorrelation, layerNumber, mapHeight, mapWidth, verticalWidth, 
                                                        // currentRowIndex = -4 to 4 row numbers surrounding bottom layer element
                                                        bottomCentreRowIndex - currentRowIndex + halfPatchWidth, inputGradientColumnNumber,
															currentRowIndex, currentColumnIndex);
                ...
                NCMSSIM_getMeanAndStd(meanMaps, stdMaps, mapHeight, mapWidth, patchwidth, layerNumber + halflayerspoint, 
                                    bottomCentreRowIndex, inputGradientColumnNumber, &y_bar, &sigma_y);
                
                float currentGradInput = NCMSSIM_gradSSIMbasedOnY(x_i, x_bar, sigma_x, y_i, y_bar, sigma_y, meanCorrelationValue, ssimValue, N);
...
__device__
void NCMSSIM_getGradientForBottomLayer(float* input, float* output, float* meanCorrelation, 
                                    int layerNumber, int rowNumber, int columnNumber, int numLayers, int mapHeight, int mapWidth, 
                                    float* gradOutput, float* gradInput, int patchwidth, int verticalWidth, 
                                    float* meanMaps, float* stdMaps)
{
    ...
    for(int rowIndex = -halfVerticalWidth * 2; rowIndex <= halfVerticalWidth * 2; rowIndex++)
    {
        ...
        for(int columnIndex = 0; columnIndex < mapWidth; columnIndex++)
        {
           ...
           //if the row and column index is valid, get the corresponding gradient
            ...
            {
                NCMSSIM_setArrayContentsToZero(currentOutputGradient, verticalWidth, mapWidth);
                
                //retrieve the output gradient for currentMapRowIndex, currentMapColumnIndex
                NCMSSIM_retrieveOutputGradient(gradOutput, currentOutputGradient, layerNumber, currentRowIndex, currentColumnIndex, mapHeight, mapWidth, patchwidth, verticalWidth);
                ...
                inputGradient += NCMSSIM_retrieveGradientWrtCorrelateeInput(input, output, meanCorrelation, currentOutputGradient, layerNumber, currentRowIndex, 
...
__global__
void NCMSSIM_calcNormCrossMapSSIMGradInput(float* input, float* output, float* meanCorrelation, 
                                        int numLayers, int mapHeight, int mapWidth, float* gradOutput, float* gradInput, 
                                        int patchwidth, int verticalWidth, float* meanMaps, float* stdMaps)
{
    //calculate the layer index, row index, column index
    ...
    if(layerNumber < halflayerspoint) {
        NCMSSIM_getGradientForTopLayer(input, output, meanCorrelation, layerNumber, rowNumber, columnNumber, 
                                        numLayers, mapHeight, mapWidth, gradOutput, gradInput, 
                                        patchwidth, verticalWidth, meanMaps, stdMaps);
    }
    else
    {
        layerNumber = layerNumber - halflayerspoint;
        NCMSSIM_getGradientForBottomLayer(input, output, meanCorrelation, layerNumber, rowNumber, columnNumber, 
                                           numLayers, mapHeight, mapWidth, gradOutput, gradInput, 
                                           patchwidth, verticalWidth, meanMaps, stdMaps);
    }
...
void NCMSSIM_updateGradInput(THCState *state, THCudaTensor *input, THCudaTensor *output, THCudaTensor *meanCorrelation, 
                            THCudaTensor *gradOutput, THCudaTensor *gradInput, int patchwidth, int verticalWidth, 
    ...
    float* meanCorrelationContents = THCudaTensor_data(state, meanCorrelation);
	float* gradOutputPtr = THCudaTensor_data(state, gradOutput);
	float* gradInputPtr = THCudaTensor_data(state, gradInput);
	float* meanMapsPtr = THCudaTensor_data(state, meanMaps);
	float* stdMapsPtr = THCudaTensor_data(state, stdMaps);

    NCMSSIM_calcNormCrossMapSSIMGradInput<<<numLayers, mapHeight * mapWidth>>>(inputContents, outputContents, meanCorrelationContents, 
                                                                        numLayers, mapHeight, mapWidth, gradOutputPtr, gradInputPtr,
                                                                        patchwidth, verticalWidth, meanMapsPtr, stdMapsPtr);
    CudaCheckError();
}
