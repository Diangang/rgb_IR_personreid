...
local nstates = {64,128,128,64,500}
...
function create_model()
    ...
    if(not opt.forceNewModel or lfs.attributes(LOAD_MODEL_NAME) ~= nil) then
        ...
        --nstates[3], nstates[3]
        spatialConfidence = nn.SpatialConvolution(12*5*nstates[2], nstates[3], 1, 1)(CCU_ReLU):annotate{
        ...
        global_summary = nn.SpatialConvolution(nstates[3], nstates[4], 3, 3)(CCU_summary_relu):annotate{
        ...
        --reshape
        reshape = nn.Reshape(nstates[4]*17*5)(global_summary_maxpool):annotate{
            name='Reshaping unit',
            graphAttributes = {color = TEXTCOLOR, style = NODESTYLE, fillcolor = COLOR_AUGMENTS}
        };

        --FC500
        FC500_XCORR = nn.Linear(nstates[4]*17*5, nstates[5])(reshape):annotate{
    ...
    -- Ahmed paper branch
       -- Joining layer to join the filtered features from two subNetworks
        ...
        patch_summary = nnpackage.SpatialConvolution(5 * 5 * 2 * nstates[2], nstates[3], 1, 1)(CINrelu):annotate{
        ...
        across_patch = nnpackage.SpatialConvolution(nstates[3], nstates[4], 3, 3)(patch_summary_relu):annotate{
        ...
        --reshape
        reshape = nn.Reshape(nstates[4]*17*5)(across_maxpool):annotate{
            name='Reshaping unit',
            graphAttributes = {color = TEXTCOLOR, style = NODESTYLE, fillcolor = COLOR_AUGMENTS}
        };

        --FC500
        FC500_CIN = nn.Linear(nstates[4]*17*5, nstates[5])(reshape):annotate{
            name='Fully connected layer - 500 nodes',
            graphAttributes = {color = TEXTCOLOR, style = NODESTYLE, fillcolor = COLOR_FC}
        };
        ...
        --FC2
        FC2 = nn.Linear(2 * nstates[5], 2)(FCReLU):annotate{
